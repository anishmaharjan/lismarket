...
export const addCategory = (category) => (dispatch) => ({
  type: ADD_CATEGORY,
  payload: API.graphql(graphqlOperation(createCategory, {input: category})).then(result => {
        dispatch({
...
export const getProductsByCategory = (categoryId) => (dispatch) => ({
  ...
  payload: API.graphql(graphqlOperation(getCategory, {id: categoryId})).then(result => {
    dispatch({
